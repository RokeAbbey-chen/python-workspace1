...
def grow_rule(pos, neg, rule=None, rules=None):
    ...
    while True:
        ...
        if max_gain <= 0.0 or bindings(neg, rules) == 0:
            print 'max_gain : {}, bindings(neg, rules) : {}'.format(max_gain, bindings(neg, rules))
            rules.pop()
            return rule
        else:
            rules.pop()
...
def irep(pos, neg):
    ...
    while pos:
        ...
        print 'rule before prune : {}'.format(rule)
        ...
        if meta['opts']['p']:
            ...
            print 'rule after prune : {}'.format(rule)
...
def optimize(pos, neg, ruleset):
    ...
    while i < len(new_ruleset):
        ...
        print 'rule : {}, rep : {}, rev : {}'.format(rule, reprule, revrule)
...
def prune_rule(pos, neg, rule, rules=None):
    ...
    while len(tmp_rule.keys()) > 1:
        ...
        # print '1 : tmp_rule {}'.format(tmp_rule)
        # print '1 : rules {}'.format(rules)
        del tmp_rule[keys[i]]
        # print '2 : tmp_rule {}'.format(tmp_rule)
        # print '2 : rules {}'.format(rules)
